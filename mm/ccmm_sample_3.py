            ...
            for tid in tids:
                ...
                for interval in intervals:
                    ...
                    for s in range(0,interval):
                        ...
                        maxd = track.max_d()
                        avgd = track.length() / len(track.rds)
                        b = in_break(maxd)
                        ...
                        f.write(' '.join([tid, interval, s]) + ':')
                        f.write(' '.join([maxd, avgd]) + ':')
                        f.write(' '.join([
                            str(result['matched_length']), 
                            str(result['missed_length']),
                            str(result['false_length'])
                            ]) + '\n')
                    print ':',
                print ""
